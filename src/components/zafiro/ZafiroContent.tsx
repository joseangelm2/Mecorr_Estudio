...
export default function ZafiroContent({ project }: Props) {
  ...
  useEffect(() => {
    const target = new Date(project.event_date).getTime();
    ...
    <div className="contenido">
      ...
      <section className="confirmacion-asistencia show-p-y">
        ...
        <form onSubmit={handleRSVP}>
          ...
          <input type="text" id="zf-familia" name="familia" maxLength={40} className="familia-input" placeholder="Escribe Tu Nombre" ref={nameRef} required />
          <label className="texto" style={{ marginTop: "2%" }}>Confirmo que:</label>
          <div>
            <input type="radio" id="zf-asistire" name="confirmacion" value="asistire" className="texto" checked={attending} onChange={() => setAttending(true)} />
            <label htmlFor="zf-asistire" className="texto"> Asistiré</label>
          </div>
          <div>
            <input type="radio" id="zf-noAsistire" name="confirmacion" value="noAsistire" className="texto" checked={!attending} onChange={() => setAttending(false)} />
