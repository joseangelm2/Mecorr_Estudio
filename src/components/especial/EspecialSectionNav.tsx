...
export default function EspecialSectionNav() {
  const [open, setOpen] = useState(false)
  const [items, setItems] = useState<typeof CANDIDATE_SECTIONS>([])

  useEffect(() => {
    setItems(CANDIDATE_SECTIONS.filter(s => document.getElementById(s.id)))
  }, [])
    ...
    <div style={{ position: 'fixed', top: '20px', left: '20px', zIndex: 9999 }}>
      <button
        ...
        style={{
          ...
          borderRadius: '50%',
          border: 'none',
          background: 'var(--inv-primary, #b08968)',
          color: '#fff',
          display: 'flex',
          alignItems: 'center',
